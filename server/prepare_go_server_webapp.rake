...
COMPRESSED_ALL_DOT_CSS = ["target/css_sass.css"]
...
task :create_all_css do
  ...
  File.open("target/all.css", "w") do |handle|
    ["main.css", "module.css"].each do |file|
      merge(handle, File.join(main_dir, file))
    end
  end

  parent_directory = "target/webapp/stylesheets"
  css_directories_to_be_compressed = [{:dir => "css_sass", :perform => Proc.new do |d| yui_compress_all(YUI_CSS_OUTPUT, File.join(parent_directory, d), "**/*.css") end}]

  css_directories_to_be_compressed.each do |tuple|
    tuple[:perform].call(tuple[:dir])
  end

  css_to_be_merged = [ "css_sass/**/*.css"]
